...
def displacement_deflection(field, mesh, gamma, eta, k=1, **kwargs):
    # xArray, yArray = fg.arrays_from_mesh(mesh)
    field_change = np.exp(1j * k * fg.rho(*mesh) * np.sin(gamma) * np.cos(fg.phi(*mesh) - eta))
    # mesh_new = np.meshgrid(xArray, yArray, indexing='ij', **kwargs)
        ...
        for p in np.arange(p1, p2 + 1):
            ...
            for l0 in np.arange(l01, l02 + 1):
                for p0 in np.arange(p01, p02 + 1):
                    element_ = LG_transition_matrix_real_space(displacement_lateral, l=l, p=p,
                                                               l0=l0, p0=p0,
...
def variance_map(beam, mesh, resolution_V=(4, 4), xBound=(-1, 1), yBound=(-1, 1), width=1., k0=1.,
    ...
    for i, x in enumerate(xArray):
        ...
        for j, y in enumerate(yArray):
            ...
            r = fg.rho(x, y)
            eta = np.angle(x + 1j * y)
            V[i, j] = variance_single_transition(beam, mesh, r, eta, width=width, k0=k0,
                                                 p=p, l=l, p0=p0, l0=l0)
    return V
...
def LG_spectrum(beam, l=(-3, 3), p=(0, 5), xM=(-1, 1), yM=(-1, 1), width=1., k0=1., mesh=None, plot=True):
    ...
    # print(modes)
    if plot:
        pl.plot_2D(np.abs(spectrum), y=np.arange(l1 - 0.5, l2 + 1 + 0.5), x=np.arange(p1 - 0.5, p2 + 1 + 0.5),
